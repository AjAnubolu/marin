...
def _values(sink: RecordingSink, metric: str) -> list[float]:
    return [s.value for s in sink.samples if s.metric == metric]
...
def test_gauge_collector_records_all_samples_stamped(caplog):
    ...
    sink = RecordingSink()
    runner = CollectorRunner(sinks=[sink])
    runner.add(Collector("gauge", lambda: [sample("m_a", 7.0, zone="z"), sample("m_b", 9.0)], timeout=1.0, cadence=0.05))
    with caplog.at_level(logging.INFO, logger="runner"):
        _run_briefly(runner)
...
def test_multiple_collectors_run_independently(caplog):
    runner = CollectorRunner()
    ...
    assert any(m.startswith("probe a: ") for m in msgs)
    assert any(m.startswith("probe b: ") for m in msgs)
