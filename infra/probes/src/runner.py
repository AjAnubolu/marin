...
def health_collector(name: str, check: Callable[[], bool], *, timeout: float, cadence: float) -> Collector:
    ...
    return Collector(
        name=name,
        collect=lambda: [sample(METRIC_UP, 1.0 if check() else 0.0, probe=name)],
        timeout=timeout,
        cadence=cadence,
    )
    ...
    async def _run_collector(self, collector: Collector) -> None:
        while True:
            ...
            try:
                ...
                samples = [sample(METRIC_UP, 0.0, probe=collector.name)]
            except Exception:
                logger.exception("collector %s raised", collector.name)
                samples = [sample(METRIC_UP, 0.0, probe=collector.name)]
            wall_ms = (time.monotonic() - start) * 1000
            samples = [*samples, sample(METRIC_LATENCY_MS, wall_ms, probe=collector.name)]
