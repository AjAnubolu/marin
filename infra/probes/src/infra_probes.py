...
"""Synthetic infra canary. Health checks against Iris and Finelog plus a TPU
provisioning-stats gauge, each run as a collector on its own cadence; samples are
logged to stdout (picked up by Cloud Logging on COS) and fanned to the sinks.
"""
...
from tpu_provisioning import collect_tpu_provisioning
...
# TPU provisioning gauge: a trailing window aggregated and re-emitted each cadence.
# A 3h window smooths the bursty per-minute noise (stockouts persist for hours);
# 15min cadence is ample resolution and the finelog query is sub-second. The
# timeout covers three bounded finelog queries plus aggregation.
TPU_PROVISION_WINDOW_HOURS = 3.0
TPU_PROVISION_CADENCE = 900.0
TPU_PROVISION_TIMEOUT = 60.0
...
def build_collectors(iris: RemoteClusterClient, finelog: LogClient, zones: tuple[str, ...]) -> list[Collector]:
    """Health checks plus the TPU provisioning gauge, each on its own cadence."""
    collectors = [
        ...
        Collector(
            name="tpu-provisioning",
            collect=lambda: collect_tpu_provisioning(finelog, window_hours=TPU_PROVISION_WINDOW_HOURS),
            timeout=TPU_PROVISION_TIMEOUT,
            cadence=TPU_PROVISION_CADENCE,
        ),
    ]
    for zone in zones:
        collectors.append(
