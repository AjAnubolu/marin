...
class Sample:
    ...
    # collected_at is None only transiently between sample() and the runner
...
def sample(metric: str, value: float, /, **labels: str) -> Sample:
    """Build an unstamped Sample with JSON-encoded labels.

    The runner stamps ``collected_at`` with the cycle's start time so all samples
    from one collection share a timestamp. Labels are sorted for stable encoding.
    """
    return Sample(metric=metric, value=float(value), labels=json.dumps(labels, sort_keys=True))
